...
import com.intergral.deep.agent.api.utils.string.lookup.StringLookup;
 ...
 * <p>
 * Variable values are typically resolved from a map, but could also be resolved from system properties, or by supplying a custom variable
 * resolver.
 * <p>
 ...
 * </pre>
 * <p>
 * Typical usage of this class follows the following pattern: First an instance is created and initialized with the map that contains the
 * values for the available variables. If a prefix and/or suffix for variables should be used other than the default ones, the appropriate
 * settings can be performed. After that the <code>replace()</code> method can be called passing in the source text for interpolation. In
 * the returned text all variable references (as long as their values are known) will be resolved. The following example demonstrates this:
 *
 * <pre>
 ...
 * </pre>
 * <p>
 * Also, this class allows to set a default value for unresolved variables. The default value for a variable can be appended to the variable
 * name after the variable default value delimiter. The default value of the variable default value delimiter is ':-', as in bash and other
 * *nix shells, as those are arguably where the default ${} delimiter set originated. The variable default value delimiter can be manually
 * set by calling {@link #setValueDelimiterMatcher(StringMatcher)}, {@link #setValueDelimiter(char)} or {@link #setValueDelimiter(String)}.
 ...
 * </pre>
 * <p>
 * In addition to this usage pattern there are some static convenience methods that cover the most common use cases. These methods can be
 * used without the need of manually creating an instance. However if multiple replace operations are to be performed, creating and reusing
 * an instance of this class will be more efficient.
 * <p>
 ...
 * Cyclic replacements are detected and will cause an exception to be thrown.
 * <p>
 ...
 * <code>name</code> variable is <code>x</code>):
 *
 * <pre>
 *   The variable ${x} must be used.
 * </pre>
 * <p>
 * To achieve this effect there are two possibilities: Either set a different prefix and suffix for variables which do not conflict with the
 * result text you want to produce. The other possibility is to use the escape character, by default '$'. If this character is placed before
 * a variable reference, this reference is ignored and won't be replaced. For example:
 *
 * <pre>
 *   The variable $${${name}} must be used.
 * </pre>
 ...
 * explicitly by setting the {@link #setEnableSubstitutionInVariables(boolean) enableSubstitutionInVariables} property to <b>true</b>.
 * <p>
...
public class StringSubstitutor {
  ...
  public StringSubstitutor(final StringLookup variableResolver) {
    this(variableResolver, DEFAULT_PREFIX, DEFAULT_SUFFIX, DEFAULT_ESCAPE, DEFAULT_VALUE_DELIMITER);
  }
   ...
   * <p>
   * The variable default value delimiter is the character or characters that delimite the variable name and the variable default value.
   * This delimiter is expressed in terms of a matcher allowing advanced variable default value delimiter matches.
   * <p>
   ...
   * <p>
   * The variable prefix is the character or characters that identify the start of a variable. This prefix is expressed in terms of a
   * matcher allowing advanced prefix matches.
   *
   ...
   * <p>
   * The variable suffix is the character or characters that identify the end of a variable. This suffix is expressed in terms of a matcher
  ...
  /**
   * Returns a flag whether substitution is disabled in variable values.If set to <b>true</b>, the values of variables can contain other
   * variables will not be processed and substituted original variable is evaluated, e.g.
   *
   * <pre>
   * Map valuesMap = HashMap();
  ...
  /**
   * Sets a flag whether substitution is done in variable names. If set to <b>true</b>, the names of variables can contain other variables
   * which are processed first before the original variable is evaluated, e.g.
  ...
  /**
   * Sets a flag controlling whether escapes are preserved during substitution. If set to <b>true</b>, the escape character is retained
  ...
  /**
   * Replaces all the occurrences of variables with their matching values from the resolver using the given source as a template. The source
  ...
  /**
   * Replaces all the occurrences of variables in the given source object with their matching values from the resolver. The input source
   * object is converted to a string using <code>toString</code> and is not altered.
   *
  ...
  /**
   * Replaces all the occurrences of variables within the given source buffer with their matching values from the resolver. The buffer is
  ...
  /**
   * Replaces all the occurrences of variables within the given source builder with their matching values from the resolver. The builder is
   * updated with the result.
   * <p>
   * Only the specified portion of the buffer will be processed. The rest of the buffer is not processed, but it is not deleted.
   *
   ...
   * <p>
   * Writers of subclasses can override this method if they need to alter how each substitution occurs. The method is passed the variable's
   * name and must return the corresponding value. This implementation uses the {@link #getStringLookup()} with the variable's name as the
   * key.
   *
   ...
   * <p>
   * The variable default value delimiter is the character or characters that delimite the variable name and the variable default value.
   * This method allows a single character variable default value delimiter to be easily set.
   *
   ...
   * <p>
   * The variable default value delimiter is the character or characters that delimite the variable name and the variable default value.
   * This method allows a string variable default value delimiter to be easily set.
   * <p>
   * If the <code>valueDelimiter</code> is null or empty string, then the variable default value resolution becomes disabled.
   *
   ...
   * <p>
   * The variable prefix is the character or characters that identify the start of a variable. This method allows a single character prefix
   * to be easily set.
   *
   ...
   * Sets the variable prefix to use.
   * <p>
   * The variable prefix is the character or characters that identify the start of a variable. This method allows a string prefix to be
   * easily set.
   *
   ...
   * <p>
   * The variable suffix is the character or characters that identify the end of a variable. This method allows a single character suffix to
   * be easily set.
   *
  ...
  /**
   * Recursive handler for multiple levels of interpolation. This is the main interpolation method, which resolves the values of all
   * variable references contained in the passed in text.
   *
   * @param buf            the string builder to substitute into, not null
   * @param offset         the start offset within the builder, must be valid
   * @param length         the length within the builder to be processed, must be valid
   * @param priorVariables the stack keeping track of the replaced variables, may be null
   * @return the length change that occurs, unless priorVariables is null when the int represents a boolean flag as to whether any change
   *     occurred.
   */
